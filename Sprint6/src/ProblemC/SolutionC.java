...
class Graph implements Iterable<Integer> {
    ...
    public void setStartVertex(int vertex) {startVertex = vertex;}
...
class DepthFirstIterator implements Iterator<Integer> {
    ...
    private final Stack<Integer> vertexQueue;
    ...
    public Integer next() {
        ...
        Iterator<Integer> reverseIterator = ((TreeSet<Integer>)adjVertexes).descendingIterator();

        while (reverseIterator.hasNext()){
            Integer vertex = reverseIterator.next();
        ...
        while (!vertexQueue.empty() && visitedVertexes.contains(vertexQueue.peek())){
            vertexQueue.pop();
        }
